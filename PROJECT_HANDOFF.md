...
## Brand, donor experience, analytics, and hospital QR update (29 August)

- Welcome/auth follow-up: replaced the sparse start page with a branded hero layout, soft background accents, feature chips and stronger visual hierarchy. Login now includes Forgot password. Recovery sends a Supabase email to `io.supabase.mydarah://reset-password`; Android handles that deep link, AuthGate detects `passwordRecovery`, and the user sets a new password before being signed out to log in again. Add this exact URI to Supabase Authentication > URL Configuration > Redirect URLs before acceptance testing.
- Do not hard-code or bulk-share a password in Flutter. Test accounts may temporarily use the same password only by completing password recovery for each controlled test email; never do this for real users or production accounts.
- Benefits follow-up: verified boundaries remain Bronze 1–5, Silver 6–15 and Gold 16+. Non-current swipe cards now say Achieved/Locked instead of displaying misleading current-tier progress. The current-level pill is at the card's top-left and available reward points were removed from the level card.
- Swiping Bronze/Silver/Gold now changes both the proposed MyDarah partner-benefit list and the relevant official PDN privilege summary below it. Proposed screening/partner benefits are clearly labelled as prototype benefits; the app does not claim an actual TAR UMT Hospital partnership. The official PDN link and official-record/citizenship/availability disclaimer remain.
- Organisation redesign follow-up: changed role accents to warm but distinct colours (donor light red, organisation burnt orange, hospital pink-red, system admin muted grey-red). The complete staff `Theme` wrapper applies the accent to pushed pages, profiles, navigation, buttons, chips, cards and progress indicators.
- Organisation Home now contains three action cards only: Manage events, Donor analysis, and Event venues. The active-event QR shortcut remains on Home. Removed the embedded blood-group bars and duplicate management buttons.
- Manage Events is grouped into expandable In progress, Upcoming, Ended, and Cancelled sections, with In progress shown first.
- Added a dedicated linked Donor Analysis screen. Its Last 5 events / Past 3 months selector controls registration and verified totals, a two-series event chart, and the blood-type pie chart together. Upcoming/in-progress registrations are included for planning but remain distinct from verified attendance.
- Benefits now uses horizontally swipeable Bronze/Silver/Gold tier cards with tier colours, progress, current-level state, donation count and points, inspired by the requested interaction while retaining PDN disclaimers and official reference information.
- Validation after this redesign: analyzer has no issues, all 27 tests pass, and the Android debug APK builds successfully.
- Fixed the organisation dashboard remote query: `organiser_name` is enriched by the app and is not a physical `donation_events` column. Requesting it directly caused the dashboard error screen.
- Applied visible role themes to the complete staff shells: organisation uses indigo, hospital uses coral, and system administration uses purple. Existing role workflows are preserved.
- Current validation after these fixes: `flutter analyze` reports no issues and all 27 tests pass.
- Follow-up: replaced the earlier M-shaped logo with the approved simple red blood-drop and white medical-cross design. Regenerated and verified the Android launcher icon on the emulator. Launcher changes require a rebuilt/reinstalled APK; hot reload does not refresh Android's cached home-screen icon.
- Fixed a shared theme layout defect where `Size.fromHeight(50)` forced every FilledButton to infinite width. Buttons inside Rows caused pages such as Donation Events to fail rendering. The theme now uses a flexible `Size(0, 50)`, and a regression widget test covers row-based buttons.
- Runtime verification: reinstalled on Pixel API 35, Supabase initialised, donor Home rendered at full resolution, Donation Events rendered successfully after the fix, analyzer reported no issues, and all 26 tests passed. Android Studio's 30% Running Devices zoom and screenshot/select overlay can make the UI look much smaller than the actual rendered screen.
- Added the approved MyDarah blood-drop and medical-cross logo as an in-app brand asset and generated Android adaptive/default launcher icons. The launcher icon generator is configured in `pubspec.yaml` and can be rerun with `dart run flutter_launcher_icons`.
- Introduced a more consistent Material 3 visual system. Role accent colours are donor green, organisation indigo, hospital coral, and system-administrator purple; database behaviour and role routing are unchanged.
- Donor Home now shows a clear green/amber/grey eligibility card, tappable donation/point/level summaries, recognition-benefit navigation, and the donor's next registered event. A registered event that is currently in progress shows a direct `Show my attendance QR` action. The QR remains event-specific and is not exposed before the event starts.
- Recognition levels are based on verified donation count, not spendable points: Bronze 1–5, Silver 6–15, Gold 16+. Redeeming points cannot reduce a donor level. The benefits screen explains that these are MyDarah recognition levels and separately shows the matching official PDN privilege category/reference; MyDarah must not claim its database is the official MOH/BBIS donor record.
- Organisation Home now includes upcoming, registration, and verified counts; an active/next-event card; a direct active-event scanner shortcut; and a blood-group registration analysis. Existing event management, manual verification, QR verification, centre/venue management, profile, feedback, and notifications remain available.
- Donor location wording now distinguishes `Official BBISv2 collection facility` from `Organisation event venue`. Organisation-managed records are labelled event venues so they are not presented as permanent official blood banks.
- Added hospital emergency QR parity. After responding to an emergency request, a donor can show a request-specific QR (`mydarah:emergency:<request UUID>:donor:<donor UUID>`). The hospital scans it from the request's donor-response page, confirms identity and completed collection, then the existing `verify_emergency_donation` RPC awards points and records the donation. Manual Verify remains as the fallback.
- Both emergency QR and manual verification now calculate the next eligible date as three calendar months from verification. The existing server RPC remains authoritative for hospital ownership, duplicates, and eligibility; do not replace those database checks with client-only checks.
- No new Supabase migration is required for this UI/QR update. It uses the existing emergency responses table and `verify_emergency_donation` RPC. Required older migrations/functions listed below still need to be deployed in any new Supabase project.
- Validation on 29 August: `dart analyze` reports no issues, all 25 Flutter tests pass, launcher icons generate successfully, and the Android debug APK builds at `build/app/outputs/flutter-apk/app-debug.apk`.
- Still perform physical acceptance tests with separate donor/organisation/hospital accounts: event QR camera scan, emergency QR camera scan, wrong-request QR, duplicate scan, ineligible donor, manual fallback, point award, three-month eligibility update, launcher appearance, and the refreshed layouts at normal device zoom.
